...
| Web app | Tautan web app yang digunakan untuk mengakses model serving: [customer-churn-model-metadata](https://customerchurnapi-z4ib9f9m.b4a.run/v1/models/customer-churn-model) |
| Monitoring | Sistem monitoring menggunakan Prometheus untuk melakukan *scraping* metrik dari TF Serving. Selama tahap pengujian berjalan, terekam lonjakan metrik customer_churn_requests_total yang menandakan bahwa sistem sanggup melayani puluhan *request* secara instan. Metrik latensi juga terukur sangat cepat karena optimasi performa TF Serving. |
