...
RUN echo '#!/bin/bash' > /usr/bin/tf_serving_entrypoint.sh && \
    ...
    echo 'exec tensorflow_model_server --port=8500 --rest_api_port=$PORT --rest_api_address=0.0.0.0 --model_name=${MODEL_NAME} --model_base_path=${MODEL_BASE_PATH}/${MODEL_NAME} --monitoring_config_file=${MONITORING_CONFIG} "$@"' >> /usr/bin/tf_serving_entrypoint.sh && \
